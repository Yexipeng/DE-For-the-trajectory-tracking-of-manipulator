...
if t(k)<TE
    qr1(k)=(q1d-q10)*(t(k)/TE-1/(2*pi)*sin(4*pi*t(k)/TE))+q10;   %不含原点的参考轨迹(1)
else 
    qr1(k)=q1d;
end
if t(k)<TE
    qr2(k)=(q2d-q20)*(t(k)/TE-1/(2*pi)*sin(4*pi*t(k)/TE))+q20;   %不含原点的参考轨迹(1)
else 
    qr2(k)=q2d;
end
if t(k)<TE
    qr3(k)=(q3d-q30)*(t(k)/TE-1/(2*pi)*sin(4*pi*t(k)/TE))+q30;   %不含原点的参考轨迹(1)
else 
    qr3(k)=q3d;
end
